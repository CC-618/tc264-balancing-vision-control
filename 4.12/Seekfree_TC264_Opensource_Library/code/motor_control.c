...
signed short int my_abs_1(signed short int a,signed short int b)
{
    if(a > b)   return  (a - b);
    else        return  (b - a);
}
...
void uart_control_callback(void)
{
    ...
    signed short int left; signed short int right;
        ...
        if(motor_value.receive_data_count >= 7)                                             // 判断是否接收到指定数量的数据
        {
            if(motor_value.receive_data_buffer[0] == 0xA5)                                  // 判断帧头是否正确
            {
                ...
                if(motor_value.sum_check_data == motor_value.receive_data_buffer[6])        // 校验数据准确性
                {
                    ...
                    {
                        left   = (((int)motor_value.receive_data_buffer[2] << 8) | (int)motor_value.receive_data_buffer[3]);  // 拟合左侧电机转速数据
                        if( my_abs_1(left, motor_value.receive_left_speed_data) < 1000) motor_value.receive_left_speed_data = left;
                        right = (((int)motor_value.receive_data_buffer[4] << 8) | (int)motor_value.receive_data_buffer[5]);  // 拟合右侧电机转速数据
                        if( my_abs_1(right, motor_value.receive_right_speed_data) < 1000) motor_value.receive_right_speed_data = right;
                    }
...
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
void small_driver_set_duty(signed short int left_duty, signed short int right_duty)
{
